<script setup>
import { ref } from "vue";
import { useModalStore, useTableDataStore } from "@/stores";
...
const { tableData, setTableData, searchResult, currentPageSearchResult, paginatedTableData, paginatedSearchResult, currentPage, totalPages, totalPagesSearchResult } = storeToRefs(useTableDataStore());
const { goToPage, goToPageSearchResult } = useTableDataStore();
...
<template>
  <div class="relative overflow-x-auto border border-gray-200 rounded-lg shadow">
    <table class="w-full text-left">
      ...
      <tbody>
        <tr v-if="searchResult.length > 0" class="bg-white border-b" v-for="table in paginatedSearchResult">
          <td class="w-4 p-4">
            <div class="flex items-center">
        ...
        <tr v-if="searchResult.length === 0" class="bg-white border-b" v-for="table in paginatedTableData">
          <td class="w-4 p-4">
            <div class="flex items-center">
              <input id="checkbox-table-1" type="checkbox" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 focus:ring-2" />
              <label for="checkbox-table-1" class="sr-only">checkbox</label>
